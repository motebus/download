> Historical Debian distribution documentation. The main AGPC download entrypoints now serve the native preview. Existing WSL bootstraps that fetch `agpc.sh` must not be used with this preview; use the explicit Debian installer URL inside an already prepared Ubuntu instead.
...
The preserved Debian installer is
**https://motebus.github.io/download/agent-sphere-apps.sh**.
Its [signature](https://motebus.github.io/download/agent-sphere-apps.sh.asc) and
[source record](https://motebus.github.io/download/agent-sphere-apps.source.json)
remain tied to the reviewed Debian release. The `agpc.sh` Pages path now serves
the separate native CLI preview. Use the explicit Debian path for this edition.
...
installation also supports basic machine chat for local login accounts.
AGPC Manager starts with Chat and everyday status screens; `/help` in uChat
shows readable, paged instructions. For optional managed team work,
use `sudo agpc-manager` → **More settings → Advanced team settings** to set the local account, create
...
`agent-computer-v0.3.0-53` native AGPC and compatibility set. It verifies the signed catalog,

...
class InstallContractTest(unittest.TestCase):
    ...
    def test_cx_installer_requires_explicit_admission_and_pinned_key(self) -> None:
        ...
        self.assertIn("--cx-bootstrap <absolute-file>", text)
        self.assertIn("--motessh-bootstrap <absolute-file>", text)
        ...
        self.assertIn('cmp -s "$CX_BOOTSTRAP" "$CX_TOPOLOGY"', text)
        self.assertIn('cmp -s "$MOTESSH_BOOTSTRAP" "$MOTESSH_TOPOLOGY"', text)
        self.assertIn('apt-get download motessh', text)
        self.assertIn('cmp -s "$MOTESSH_BOOTSTRAP" "$signed_motessh_topology"', text)
    ...
    def test_cx_one_installation_closes_runtime_and_lifecycle_checks(self) -> None:
        text = CX_INSTALLER.read_text(encoding="utf-8")
        self.assertIn("openssh-client openssh-server", text)
        self.assertIn("motestream motessh moterdp cx-node", text)
        self.assertIn("doctor()", text)
        self.assertIn("sshd -t", text)
        self.assertIn("/usr/bin/motessh --help", text)
        self.assertIn("/usr/bin/cx-node --check-config", text)
        self.assertIn('systemctl show "$unit_name" -p NRestarts --value', text)
        self.assertIn('sleep "$SERVICE_STABILITY_SECONDS"', text)
        self.assertIn("re-run this install command to upgrade", text)
        self.assertIn("apt-get remove -y --no-auto-remove cx-node", text)
